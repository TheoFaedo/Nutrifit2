...
const ProfileSVG: FunctionComponent<Props> = (props) => {
    ...
    const {fillPrimary, fillSecondary, stroke, className} = props;

    const classAtt = className ?? 'h-7';
    const primaryFill = fillPrimary ?? 'fill-none';
    const secondaryFill = fillSecondary ?? 'fill-none';
    const strokeColor = stroke ?? 'stroke-black';

    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 374 374" className={classAtt} role="button">
            ...
            <circle cx="187" cy="187" r="172" fill="none" stroke="black" strokeMiterlimit="10" strokeWidth="30" className={strokeColor}/>
        </svg>
    );
}
